//config
export const seoConfigHome = {
  title: 'Achraf Dev | Web Developer | Web Designer | Freelancer',
  description:
    "Hi there, I'm Achraf, a Front End Web Developer and Web Designer. I build powerful and awesome websites using the latest web technologies for businesses around the globe. If you need a modern and powerful website, send me an email.",
  keywords:
    'Web Designer, Web Developer, AchrafDev, Achraf Elmouhib, Freelancer, React, Redux, Javascript, Typescript, Design, Development, SEO, SSR, Portfolio Website, Achraf Dev, Websites, HTML, CSS, Modern Website',
};

export const seoConfigNotFound = {
  title: 'Achraf Dev | Not Found',
  description:
    "We couldn't find the page you're looking for. Please check you spelled the url correctly and if there wasn't any network issues.",
};
